/*********************************************************
* Example to demonstrate the usage of the software timer
* By: Mateus Sousa
*********************************************************/
...
void setup(){
  ...
  xTimer1 = xTimerCreate("TIMER1",pdMS_TO_TICKS(1000),pdTRUE,0,callBackTimer1); //Autoreload
  xTimer2 = xTimerCreate("TIMER2",pdMS_TO_TICKS(10000),pdFALSE,0,callBackTimer2); //OneShot
  ...
  while(1){
    if((digitalRead(BT) == LOW) && (xTimerIsTimerActive(xTimer2) == pdFALSE)){
      ...
      if(debouncingTime >= 10){
        ...
        xTimerStop(xTimer1,0); // Stop blink LED
...
void callBackTimer2(TimerHandle_t xTimer){
  ...
  xTimerStart(xTimer1,0); // Start blinking LED
}